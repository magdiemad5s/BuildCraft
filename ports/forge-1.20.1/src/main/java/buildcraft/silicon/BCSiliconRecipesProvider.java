...
public class BCSiliconRecipesProvider extends RecipeProvider{
	...
	public BCSiliconRecipesProvider(PackOutput output) {
        super(output);
    }
	...
	protected void buildRecipes(Consumer<FinishedRecipe> writer) { 
        ...
        if (BCSiliconItems.PLUG_FACADE_ITEM.isPresent()) {
            saveSerializerOnlyRecipe(
                writer,
                new ResourceLocation(BCSilicon.MODID, "special/facade"),
                BCSiliconRecipes.FACADE_SERIALIZER.get()
            );
            saveSerializerOnlyRecipe(
                writer,
                buildcraft.silicon.recipe.FacadeSwapRecipe.ID,
                BCSiliconRecipes.FACADE_SWAP_SERIALIZER.get()
            );
        }
