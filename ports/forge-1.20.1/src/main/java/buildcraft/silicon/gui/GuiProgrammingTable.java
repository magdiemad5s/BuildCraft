...
public class GuiProgrammingTable extends GuiBC8<ContainerProgrammingTable> {
    ...
    protected void drawBackgroundLayer(PoseStack pose, int mouseX, int mouseY, float partialTicks) {
        ...
        TileProgrammingTable_Neptune tile = container.getTile();
        ...
        int selected = tile == null ? -1 : tile.selectedOption;
        if (selected >= 0 && selected < TileProgrammingTable_Neptune.OPTION_COUNT) {
            ICON_SELECTED.drawAt(getActiveGraphics(), new GuiRectangle(optionX(selected), optionY(selected), 16, 16).offset(mainGui.rootElement));
        }
    }

    @Override
    protected void drawForegroundLayer(PoseStack pose, int mouseX, int mouseY) {
        getActiveGraphics().drawString(font, title, leftPos + (imageWidth - font.width(title)) / 2, topPos + 15, 0x404040, false);
        getActiveGraphics().drawString(font, playerInventoryTitle, leftPos + 8, topPos + imageHeight - 97, 0x404040, false);
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int mouseButton) {
        ...
        for (int i = 0; i < TileProgrammingTable_Neptune.OPTION_COUNT; i++) {
            ...
            if (mouseX >= x && mouseX < x + 16 && mouseY >= y && mouseY < y + 16) {
                container.sendSelectOption(tile.selectedOption == i ? -1 : i);
                return true;
            }
        }
        return flag;
