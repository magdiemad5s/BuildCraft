...
public class GateLogic implements IGate, IWireEmitter, IRedstoneStatementContainer {
    ...
    public void readConfigData(CompoundTag nbt) {
        ...
        for (int i = 0; i < connections.length; i++) {
            connections[i] = ((c >>> i) & 1) == 1
                && GateMenuStatePolicy.isSelectableConnectionIndex(variant.numSlots, isSplitInTwo(), i);
        }
    ...
    public void readPayload(FriendlyByteBuf buffer, LogicalSide side, NetworkEvent.Context ctx) throws IOException {
        ...
        if (id == NET_ID_CHANGE) {
            ...
            if (side == LogicalSide.SERVER && !canAcceptServerStatementChange(ctx)) {
                throw new InvalidInputDataException("Unauthorized gate statement update");
            }

            StatementPair pair = statements[slot];
            boolean accepted = true;
            if (isAction) {
                FullStatement<ActionWrapper> incoming =
                    new FullStatement<>(ActionType.INSTANCE, variant.numActionArgs, null);
                incoming.readFromBuffer(buffer);
                if (side == LogicalSide.SERVER) {
                    accepted = isValidClientStatement(
                        incoming,
                        getAllValidActions(),
                        variant.numActionArgs
                    );
                }
                if (accepted) {
                    applyStatementSnapshot(pair.action, incoming);
                }
            } else {
                FullStatement<TriggerWrapper> incoming =
                    new FullStatement<>(TriggerType.INSTANCE, variant.numTriggerArgs, null);
                incoming.readFromBuffer(buffer);
                if (side == LogicalSide.SERVER) {
                    accepted = isValidClientStatement(
                        incoming,
                        getAllValidTriggers(),
                        variant.numTriggerArgs
                    );
                }
                if (accepted) {
                    applyStatementSnapshot(pair.trigger, incoming);
                }
            }
            ...
            if (side == LogicalSide.SERVER) {
                ...
                // Echo either the accepted state or the unchanged authoritative state back to every open gate viewer.
                sendStatementUpdate(isAction, slot);
            }
            BCLog.d("rec " + (isAction ? pair.action : pair.trigger).writeToNbt());
            return;
        }
    ...
    private boolean canAcceptServerStatementChange(NetworkEvent.Context ctx) {
        ServerPlayer sender = ctx == null ? null : ctx.getSender();
        ContainerGate menu = sender != null && sender.containerMenu instanceof ContainerGate gateMenu
            ? gateMenu
            : null;
        return GateMenuStatePolicy.canMutateServerGate(
            sender != null,
            menu != null,
            menu != null && menu.gate == this,
            menu != null && menu.stillValid(sender)
        );
    }

    private static <S extends IStatement> boolean isValidClientStatement(
        FullStatement<S> incoming,
        SortedSet<? extends IStatement> offered,
        int supportedParameters
    ) {
        S statement = incoming.get();
        boolean parametersCanonical = true;
        if (statement == null) {
            for (IStatementParameter parameter : incoming.getParameters()) {
                parametersCanonical &= parameter == null;
            }
        } else {
            int statementParameters = statement.maxParameters();
            if (statementParameters < 0) {
                parametersCanonical = false;
            }
            for (int index = 0; index < incoming.getParamCount(); index++) {
                IStatementParameter parameter = incoming.get(index);
                if (index >= statementParameters) {
                    parametersCanonical &= parameter == null;
                } else {
                    parametersCanonical &= incoming.canSet(index, parameter);
                }
            }
        }
        return GateMenuStatePolicy.isValidStatementSelection(
            statement == null,
            statement != null && offered.contains(statement),
            statement == null ? 0 : statement.minParameters(),
            supportedParameters,
            parametersCanonical
        );
    }

    private static <S extends IStatement> void applyStatementSnapshot(
        FullStatement<S> target,
        FullStatement<S> incoming
    ) {
        target.set(incoming.get());
        for (int index = 0; index < target.getParamCount(); index++) {
            target.set(index, incoming.get(index));
        }
    }
    ...
    public void synchronizeConfiguration() {
        ...
        markGateDirty();
        for (int slot = 0; slot < statements.length; slot++) {
            sendStatementUpdate(false, slot);
            sendStatementUpdate(true, slot);
        }
        sendResolveData();
    }
