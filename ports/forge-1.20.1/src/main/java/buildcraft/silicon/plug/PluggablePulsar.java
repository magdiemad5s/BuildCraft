...
public class PluggablePulsar extends PipePluggable {
    ...
    public PluggablePulsar(PluggableDefinition definition, IPipeHolder holder, Direction side, CompoundTag nbt) {
        ...
        gateEnabledTicks = nonNegative(nbt.getInt("gateEnabledTicks"));
        gateSinglePulses = nonNegative(nbt.getInt("gateSinglePulses"));
        pulseStage = sanitizePulseStage(nbt.getInt("pulseStage"));
    }
    ...
    private void readData(FriendlyByteBuf buffer) {
        ...
        pulseStage = sanitizePulseStage(buffer.readByte());
    }
    ...
    static int incrementSaturated(int value) {
        return value == Integer.MAX_VALUE ? Integer.MAX_VALUE : nonNegative(value) + 1;
    }

    private static int sanitizePulseStage(int value) {
        return MathUtil.clamp(value, 0, PULSE_STAGE - 1);
    }
    ...
    public void onTick() {
        ...
        int previousPulseStage = pulseStage;
        int previousEnabledTicks = gateEnabledTicks;
        int previousSinglePulses = gateSinglePulses;
        ...
        if (pulseStage >= PULSE_STAGE) {
            pulseStage = 0;
            if (holder.getPipe().getBehaviour() instanceof IMjRedstoneReceiver rsRec) {
        ...
        if (pulseStage != previousPulseStage || gateEnabledTicks != previousEnabledTicks
            || gateSinglePulses != previousSinglePulses) {
            markDirty();
        }
        if (isOn != lastPulsing) {
            lastPulsing = isOn;
    ...
    public void addSinglePulse() {
        int next = incrementSaturated(gateSinglePulses);
        if (next != gateSinglePulses) {
            gateSinglePulses = next;
            markDirty();
        }
    }

    private void markDirty() {
        if (holder.getPipeTile() != null) {
