...
public class TileProgrammingTable_Neptune extends TileLaserTableBase implements MenuProvider {
    ...
    public void load(CompoundTag nbt) {
        super.load(nbt);
        selectedOption = nbt.contains("selectedOption", Tag.TAG_ANY_NUMERIC) ? nbt.getInt("selectedOption") : -1;
        refreshOptions();
    }
    ...
    public void writePayload(int id, FriendlyByteBuf buffer, LogicalSide side) {
        ...
        if (side == LogicalSide.SERVER && (id == NET_GUI_DATA || id == NET_SELECTED_OPTION)) {
    ...
    public void readPayload(int id, FriendlyByteBuf buffer, LogicalSide side, NetworkEvent.Context ctx) throws IOException {
        ...
        if (id == NET_GUI_DATA || id == NET_SELECTED_OPTION) {
            if (side != LogicalSide.CLIENT) {
                throw new IOException("Rejected serverbound Programming Table selection snapshot");
            }
            int option = buffer.readVarInt();
            selectedOption = option >= 0 && option < OPTION_COUNT
                ? option
                : -1;
            refreshOptions();
        }
