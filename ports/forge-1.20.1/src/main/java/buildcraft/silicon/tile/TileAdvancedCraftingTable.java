...
import buildcraft.silicon.container.AdvancedCraftingRecipeSelection;
...
public class TileAdvancedCraftingTable extends TileLaserTableBase implements IAutoCraft, MenuProvider, IMjRedstoneReceiver {
    ...
    public void update() {
        ...
        if (crafting.canCraft() && power >= POWER_REQ && crafting.craft()) {
    ...
    public void readPayload(int id, FriendlyByteBuf buffer, LogicalSide side, NetworkEvent.Context ctx)
        ...
        // The assumed result is synchronized by the open menu's display slot.
    }

    @Override
    public void writePayload(int id, FriendlyByteBuf buffer, LogicalSide side) {
        super.writePayload(id, buffer, side);
        if (side == LogicalSide.SERVER && id == NET_GUI_DATA) {
            resultClient.setStackInSlot(0, crafting.getAssumedResult());
        }
    }
    ...
    public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
        return new ContainerAdvancedCraftingTable(
            id,
            inventory,
            invMaterials,
            invResults,
    ...
    /**
     * Applies a recipe-book selection on the logical server. The recipe has
     * already been resolved from the server recipe manager by the open menu.
     */
    public boolean applyRecipeSelection(CraftingRecipe recipe) {
        if (level == null || level.isClientSide
            || recipe.getResultItem(level.registryAccess()).isEmpty()) {
            return false;
        }

        NonNullList<ItemStack> blueprint = AdvancedCraftingRecipeSelection
            .createBlueprint(recipe, invMaterials)
            .orElse(null);
        if (blueprint == null || blueprint.size() != invBlueprint.getSlots()) {
            return false;
        }

        for (int slot = 0; slot < blueprint.size(); slot++) {
            if (!invBlueprint.canSet(slot, blueprint.get(slot))) {
                return false;
            }
        }
        for (int slot = 0; slot < blueprint.size(); slot++) {
            invBlueprint.setStackInSlot(slot, blueprint.get(slot).copy());
        }
        return true;
    }
