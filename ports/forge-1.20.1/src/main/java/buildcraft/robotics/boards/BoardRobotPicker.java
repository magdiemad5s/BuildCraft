...
public class BoardRobotPicker extends RedstoneBoardRobot {
    ...
    private void fetchNewItem() {
        ...
        if (station == null) {
            // A picker without its home station must not fall back to an unrestricted 250-block search. Use the
            // station-independent sleep AI so the board safely idles instead of restarting and draining its battery.
    ...
    public void delegateAIEnded(AIRobot ai) {
        if (ai instanceof AIRobotFetchItem) {
            ...
            startDelegateAI(new AIRobotGotoSleep(robot));
        } else if (ai instanceof AIRobotGotoSleep || ai instanceof AIRobotSleep) {
            terminate();
        }
    }
