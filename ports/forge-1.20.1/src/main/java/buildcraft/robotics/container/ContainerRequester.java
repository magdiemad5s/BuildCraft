...
public class ContainerRequester extends ContainerBCTile<TileRequester> {
        ...
        if (slotId >= 0 && slotId < slots.size()) {
            ...
            if (slot instanceof SlotRequesterTemplate templateSlot) {
                if (!player.level().isClientSide) {
                    handleTemplateClick(templateSlot, clickType, player);
                }
                return;
            }
        }
    ...
    private void handleTemplateClick(SlotRequesterTemplate slot, ClickType clickType, Player player) {
        TileRequester requester = getTile();
        if (requester == null || !requester.canInteractWith(player)) {
            return;
        }
        if (clickType == ClickType.CLONE) {
            if (player.getAbilities().instabuild && getCarried().isEmpty() && slot.hasItem()) {
        ...
        requester.setRequest(slot.handlerIndex, toSet);
        slot.setChanged();
    }
