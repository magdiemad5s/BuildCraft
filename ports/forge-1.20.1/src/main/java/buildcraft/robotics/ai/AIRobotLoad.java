...
public class AIRobotLoad extends AIRobot {
    ...
    public static ItemStack takeSingle(DockingStation station, IStackFilter filter, boolean doTake) {
        return takeMatching(station, filter, 1, doTake);
    }

    /**
     * Extracts up to {@code maxCount} matching items while respecting the station gate and sided-inventory rules.
     * Simulation and execution traverse the same slots, so robot tool searches cannot promise an item they are not
     * actually allowed to remove.
     */
    public static ItemStack takeMatching(DockingStation station, IStackFilter filter, int maxCount, boolean doTake) {
        if (station == null || filter == null || maxCount <= 0) {
        ...
        for (int slot : getSlots(container, side)) {
            ...
            if (doTake) {
                ItemStack removed = container.removeItem(slot, toTake);
                toTake = removed.getCount();
                if (toTake <= 0) {
                    continue;
                }
            }
