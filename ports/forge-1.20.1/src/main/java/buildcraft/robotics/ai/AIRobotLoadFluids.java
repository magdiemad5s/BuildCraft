...
public class AIRobotLoadFluids extends AIRobot {
    ...
    public static int load(EntityRobotBase robot, DockingStation station, IFluidFilter filter, boolean doLoad) {
        ...
        if (doLoad && filled < drained.getAmount()) {
            // Another mod's handler may return a different result for EXECUTE than it did for SIMULATE. Return every
            // unaccepted millibucket to the source instead of silently deleting it.
            FluidStack remainder = drained.copy();
            remainder.setAmount(drained.getAmount() - filled);
            int restored = handler.fill(remainder, FluidAction.EXECUTE);
            if (restored < remainder.getAmount()) {
                buildcraft.api.core.BCLog.logger.error(
                    "Robot fluid load could not return {} mB to a non-transactional source handler",
                    remainder.getAmount() - restored
                );
            }
        }
        return filled;
