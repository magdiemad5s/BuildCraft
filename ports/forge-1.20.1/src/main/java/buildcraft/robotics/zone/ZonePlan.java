...
public class ZonePlan implements IZone {
    ...
    public ZonePlan readFromByteBuf(FriendlyByteBuf buf) {
        return readFromByteBuf(buf, ZoneMapRequestPolicy.MAX_ZONE_CHUNKS_PER_EDIT);
    }

    public ZonePlan readFromByteBuf(FriendlyByteBuf buf, int maxChunks) {
        if (maxChunks < 0) {
            throw new IllegalArgumentException("maxChunks must be non-negative");
        }
        int size = buf.readInt();
        if (size < 0 || size > maxChunks) {
            throw new DecoderException("Invalid Zone Planner chunk count: " + size);
        }
        HashMap<ChunkPos, ZoneChunk> decoded = new HashMap<>();
        for (int i = 0; i < size; i++) {
            ChunkPos key = new ChunkPos(buf.readInt(), buf.readInt());
            ZoneChunk value = new ZoneChunk().readFromByteBuf(buf);
            if (decoded.putIfAbsent(key, value) != null) {
                throw new DecoderException("Duplicate Zone Planner chunk " + key);
            }
        }
        chunkMapping.clear();
        chunkMapping.putAll(decoded);
        return this;
    }
    ...
    public int getNetworkEncodedSize() {
        ...
        for (Map.Entry<ChunkPos, ZoneChunk> entry : chunkMapping.entrySet()) {
            int next = Integer.BYTES * 2 + entry.getValue().getNetworkEncodedSize();
            if (bytes > ZoneMapRequestPolicy.MAX_ZONE_PACKET_BYTES - next) {
                return ZoneMapRequestPolicy.MAX_ZONE_PACKET_BYTES + 1;
            }
            bytes += next;
        }
        return bytes;
    ...
    public void writeToByteBuf(FriendlyByteBuf buf) {
        ...
        int bytes = Integer.BYTES;
        for (Map.Entry<ChunkPos, ZoneChunk> entry : chunkMapping.entrySet()) {
            if (size >= ZoneMapRequestPolicy.MAX_ZONE_CHUNKS_PER_EDIT) {
                break;
            }
            int next = Integer.BYTES * 2 + entry.getValue().getNetworkEncodedSize();
            if (bytes > ZoneMapRequestPolicy.MAX_ZONE_PACKET_BYTES - next) {
