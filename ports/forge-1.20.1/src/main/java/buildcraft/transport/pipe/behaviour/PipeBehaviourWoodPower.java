...
public class PipeBehaviourWoodPower extends PipeBehaviour {
    ...
    public int getTextureIndex(Direction face) {
        ...
        if (pipe.getConnectedPipe(face) != Pipe.EMPTY) {
            return 0;
        }
        ...
        if (pipe.getFlow() instanceof PipeFlowRedstoneFlux) {
            IEnergyStorage storage =
                tile.getCapability(ForgeCapabilities.ENERGY, face.getOpposite()).orElse(null);
            return storage != null && storage.canExtract() ? 0 : 1;
        }
