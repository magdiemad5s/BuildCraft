...
public class PipeFlowFluids extends PipeFlow implements IFlowFluid, IDebuggable {
    ...
    public PipeFlowFluids(IPipe pipe, CompoundTag nbt) {
        ...
        if (nbt.contains("fluid")) {
            setFluid(LegacyFluidCompat.loadFluidStackFromNBT(nbt.getCompound("fluid")));
        } else {
        ...
        for (EnumPipePart part : EnumPipePart.VALUES) {
            ...
            if (nbt.contains("tank[" + direction + "]")) {
                ...
                if (compound.contains("FluidType")) {
                    FluidStack stack = LegacyFluidCompat.loadFluidStackFromNBT(compound);
    ...
    public <T> @NotNull LazyOptional<T> getCapability(@Nonnull Capability<T> capability, Direction facing) {
        if (capability == CapUtil.CAP_FLUIDS) {
            return getCachedCapability(
                CapUtil.CAP_FLUIDS, facing, sections.get(EnumPipePart.fromFacing(facing))
            ).cast();
        }
    ...
    public FluidStack tryExtractFluid(int millibuckets, Direction from, FluidStack filter, FluidAction simulate) {
        FluidExtractor extractor = (mb, c, handler) -> {
            FluidStack f = filter == null || filter.isEmpty() ? c : filter;
    ...
    public FluidStack extractFluidsForce(int min, int max, @Nullable Direction section, FluidAction simulate) {
        ...
        if (simulate.execute()) {
            s.amount -= amount;
            s.incomingTotalCache = FluidSectionAccounting.reconcileIncoming(
                s.amount, s.currentTime, s.incoming
            );
            if (s.amount == 0) {
    ...
    private void moveFromCenter() {
        ...
        if (realDirections.size() > 0) {
            ...
            float min = (float) FlowRateMath.equalShareFactor(flowRate, set.size(), totalAvailable);
            if (min <= 0) {
                return;
            }
    ...
    private void moveToCenter() {
        ...
        transferInCount = FlowRateMath.clampOffersAndCountPositive(inputPerTick, totalOffered);
        ...
        float min = (float) FlowRateMath.equalShareFactor(flowRate, transferInCount, spaceAvailable);
        for (EnumPipePart part : EnumPipePart.FACES) {
            Section section = sections.get(part);
    ...
    class Section implements IFluidHandler {
        ...
        int getMaxDrained() {
            return FluidSectionAccounting.maxDrained(amount, incomingTotalCache, fluidTransferInfo.transferPerTick);
        }
		...
		public @NotNull FluidStack getFluidInTank(int tank) {
			...
			return currentFluid.isEmpty() || amount <= 0
				? FluidStack.EMPTY
		...
		public boolean isFluidValid(int tank, @NotNull FluidStack stack) {
			...
			return !stack.isEmpty()
				&& getCurrentDirection().canInput()
				&& (currentFluid.isEmpty() || FluidCompatRegistry.areEquivalent(currentFluid, stack));
		}

		private void validateTankIndex(int tank) {
			if (tank != 0) {
				throw new IndexOutOfBoundsException("Fluid pipe section only has tank 0, requested " + tank);
			}
		}
    }
