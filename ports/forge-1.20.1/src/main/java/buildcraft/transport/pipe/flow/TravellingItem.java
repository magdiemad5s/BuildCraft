...
public class TravellingItem {
    ...
    public TravellingItem(CompoundTag nbt, long tickNow) {
        ...
        if (!Double.isFinite(speed) || speed < 0.001) {
    ...
    public void genTimings(long now, double distance) {
        ...
        double safeDistance = Double.isFinite(distance) ? Math.max(0, distance) : 0;
        tickStarted = now;
        timeToDest = Math.max(1, (int) Math.ceil(safeDistance / speed));
        tickFinished = now + timeToDest;
    }

    private double getProgress(long tick, double partialTicks) {
        long duration = tickFinished - tickStarted;
        if (duration <= 0) {
            return tick >= tickFinished ? 1 : 0;
        }
        double progress = ((tick - tickStarted) + partialTicks) / duration;
    ...
    public Vec3 interpolatePosition(Vec3 start, Vec3 end, long tick, float partialTicks) {
        double interpMul = getProgress(tick, partialTicks);
    ...
    public Vec3 getRenderPosition(BlockPos pos, long tick, float partialTicks, PipeFlowItems flow) {
        double interp = getProgress(tick, partialTicks);
