...
public class PipeBehaviourLimiter extends PipeBehaviour {
    ...
    @PipeEventHandler
    public void configureRf(PipeEventRedstoneFlux.Configure event) {
        if (limitShift == MAX_SHIFT) {
            event.disableTransfer();
        } else {
            event.setMaxPower(event.getMaxPower() >> limitShift);
    ...
    public boolean onPipeActivate(Player player, BlockHitResult trace, Level level, EnumPipePart part) {
        ...
        if (!level.isClientSide()) {
            ...
            if (pipe.getFlow() instanceof IFlowRedstoneFlux) {
                ...
                player.displayClientMessage(Component.translatable("chat.pipe.rf.iron.mode", limit), true);
            } else {
                PowerTransferInfo transferInfo = PipeApi.getPowerTransferInfo(pipe.getDefinition());
                long limit = limitShift == MAX_SHIFT
                    ? 0
                    : (transferInfo.transferPerTick >> limitShift) / MjAPI.MJ;
    ...
    private void requestReconfigure() {
        ...
        } else if (pipe.getFlow() instanceof IFlowRedstoneFlux rfFlow) {
            rfFlow.reconfigure();
            pipe.getHolder().scheduleNetworkUpdate(PipeMessageReceiver.BEHAVIOUR);
        }
    }
