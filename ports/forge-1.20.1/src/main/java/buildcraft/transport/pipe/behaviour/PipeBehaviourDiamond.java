...
public abstract class PipeBehaviourDiamond extends PipeBehaviour implements MenuProvider{
    ...
    protected void onFilterSlotChange(IItemHandlerModifiable itemHandler, int slot, ItemStack before, ItemStack after) {
        ...
        var owner = pipe.getHolder().getOwner();
        if (count >= FILTERS_PER_SIDE - 2 && owner != null && owner.getId() != null) {
            AdvancementUtil.unlockAdvancement(owner.getId(), ADVANCEMENT_NEED_LIST);
        }
    }
	...
	public void rotate(Rotation rot) {
		...
		for(Direction face : Direction.values()) {
			...
			for(int i = 0 ; i < FILTERS_PER_SIDE ;i++) {
				filters.stacks.set(i + rotate.ordinal() * FILTERS_PER_SIDE,
					copy.get(i + face.ordinal() * FILTERS_PER_SIDE).copy());
			}
		}
	}
