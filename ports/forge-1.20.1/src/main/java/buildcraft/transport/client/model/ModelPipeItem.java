...
public enum ModelPipeItem implements BakedModel {
    ...
    static {
        ...
        // Different sprite for any of the 3 sections
        {
            ...
            MutableQuad[] top = new MutableQuad[6];
            Vector3f topCenter = new Vector3f(0.5f, 0.875f, 0.5f);
            Vector3f topRadius = new Vector3f(0.25f, 0.125f, 0.25f);
            UvFaceData topUvsY = UvFaceData.from16(4, 4, 12, 12);
            UvFaceData topUvsXZ = UvFaceData.from16(4, 0, 12, 4);
            for (Direction face : Direction.values()) {
                if (face == Direction.DOWN) {
                    continue;
                }
                UvFaceData uvs = face.getAxis() == Axis.Y ? topUvsY : topUvsXZ;
                top[face.ordinal()] = ModelUtil.createFace(face, topCenter, topRadius, uvs);
            }
            QUADS_DIFFERENT[INDEX_TOP] = top;

            MutableQuad[] center = new MutableQuad[6];
            Vector3f centerPos = new Vector3f(0.5f, 0.5f, 0.5f);
            Vector3f centerRadius = new Vector3f(0.25f, 0.25f, 0.25f);
            UvFaceData centerUvs = UvFaceData.from16(4, 4, 12, 12);
            for (Direction face : Direction.values()) {
                if (face.getAxis() == Axis.Y) {
                    continue;
                }
                center[face.ordinal()] = ModelUtil.createFace(
                    face,
                    centerPos,
                    centerRadius,
                    centerUvs
            ...
            QUADS_DIFFERENT[INDEX_CENTER] = center;

            MutableQuad[] bottom = new MutableQuad[6];
            Vector3f bottomCenter = new Vector3f(0.5f, 0.125f, 0.5f);
            Vector3f bottomRadius = new Vector3f(0.25f, 0.125f, 0.25f);
            UvFaceData bottomUvsY = UvFaceData.from16(4, 4, 12, 12);
            UvFaceData bottomUvsXZ = UvFaceData.from16(4, 12, 12, 16);
            for (Direction face : Direction.values()) {
                if (face == Direction.UP) {
                    continue;
                }
                UvFaceData uvs = face.getAxis() == Axis.Y ? bottomUvsY : bottomUvsXZ;
                bottom[face.ordinal()] = ModelUtil.createFace(
                    face,
                    bottomCenter,
                    bottomRadius,
                    uvs
            ...
            QUADS_DIFFERENT[INDEX_BOTTOM] = bottom;
        }
    ...
    private static List<BakedQuad> getQuads(PipeFaceTex center, PipeFaceTex top, PipeFaceTex bottom,
        ...
        int baseQuadCount = getBaseGeometryQuadCount(center, top, bottom);
        List<BakedQuad> quads = new ArrayList<>(baseQuadCount + 6);
    ...
    static int getBaseGeometryQuadCount(PipeFaceTex center, PipeFaceTex top, PipeFaceTex bottom) {
        if (center.equals(top) && center.equals(bottom)) {
            return countNonNull(QUADS_SAME) * center.getCount();
        }
        return countNonNull(QUADS_DIFFERENT[INDEX_BOTTOM]) * bottom.getCount()
            + countNonNull(QUADS_DIFFERENT[INDEX_CENTER]) * center.getCount()
            + countNonNull(QUADS_DIFFERENT[INDEX_TOP]) * top.getCount();
    }

    private static int countNonNull(MutableQuad[] quads) {
        int count = 0;
