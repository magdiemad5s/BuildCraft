...
public enum EnumWireBetween {
    ...
    public EnumWireBetween rotate(Rotation rotation) {
        ...
        EnumWireBetween exactRotation = findExactRotation(rotation);
    ...
    private EnumWireBetween findExactRotation(Rotation rotation) {
        Direction rotatedTo = to == null ? null : rotation.rotate(to);
        EnumWirePart rotatedFirst = parts[0].rotate(rotation);
        EnumWirePart rotatedSecond = parts[1].rotate(rotation);
        for (EnumWireBetween candidate : VALUES) {
            if (candidate.to != rotatedTo) {
                continue;
            }
            boolean sameOrder =
                candidate.parts[0] == rotatedFirst && candidate.parts[1] == rotatedSecond;
            boolean reverseOrder =
                candidate.parts[1] == rotatedFirst && candidate.parts[0] == rotatedSecond;
