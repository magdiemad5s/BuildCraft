...
public class ContainerFilteredBuffer_BC8 extends ContainerBCTile<TileFilteredBuffer> {
    public final SlotPhantom[] filterSlots = new SlotPhantom[9];
    public final SlotBase[] bufferSlots = new SlotBase[9];
    ...
    public ContainerFilteredBuffer_BC8(int containerId, Inventory inventory, IItemHandlerAdv invFilter, IItemHandlerAdv invMain, ContainerLevelAccess access) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            SlotPhantom filterSlot = new SlotPhantom(invFilter, i, 8 + i * 18, 27) {
                @Override
            ...
            filterSlots[i] = filterSlot;
            addSlot(filterSlot);
            // Filtered Buffer inventory slots
            SlotBase bufferSlot = new SlotBase(invMain, i, 8 + i * 18, 61);
            bufferSlots[i] = bufferSlot;
            addSlot(bufferSlot);
        }
    }
}
