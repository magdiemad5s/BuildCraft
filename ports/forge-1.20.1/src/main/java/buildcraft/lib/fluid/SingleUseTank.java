...
public class SingleUseTank extends FluidTank {
    ...
    public FluidTank readFromNBT(CompoundTag nbt) {
        CompoundTag normalizedTank = LegacyFluidCompat.normalizeForLoad(nbt);
        FluidStack loaded = FluidCompatRegistry.canonicalize(
            LegacyFluidCompat.loadFluidStackFromNBT(nbt.getCompound(NBT_ACCEPTED_FLUID))
        );
        acceptedFluid = loaded.isEmpty() ? null : loaded;
        return super.readFromNBT(normalizedTank);
    }
