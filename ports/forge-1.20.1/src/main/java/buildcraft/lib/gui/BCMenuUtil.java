...
public final class BCMenuUtil {
    ...
    public static ItemStack quickMoveStack(AbstractContainerMenu menu, Player player, int index) {
        ...
        if (!canQuickMoveFrom(slot, player) || !slot.hasItem()) {
            return ItemStack.EMPTY;
        }
    ...
    static boolean canQuickMoveFrom(Slot slot, Player player) {
        return slot != null && isQuickMoveSourceType(slot.getClass()) && slot.mayPickup(player);
    }

    static boolean isQuickMoveSourceType(Class<?> slotType) {
        return slotType != null
            && !RecordSlot.class.isAssignableFrom(slotType)
            && !IPhantomSlot.class.isAssignableFrom(slotType)
            && !SlotDisplay.class.isAssignableFrom(slotType);
    }

    private static boolean isFakeSlot(Slot slot) {
        return slot instanceof RecordSlot || slot instanceof IPhantomSlot || slot instanceof SlotDisplay;
    }
}
