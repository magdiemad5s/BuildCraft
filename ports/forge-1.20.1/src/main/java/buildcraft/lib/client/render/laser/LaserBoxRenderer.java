...
//    	BCLog.logger.debug("LaserBoxRenderer.renderLaserBoxStatic:call unimplemented method");
    ...
    public static void renderLaserBoxStaticRelative(PoseStack pose, Matrix4f matrix, Box box, LaserType type,
        boolean center, Vec3 renderOrigin) {
        if (box == null || box.min() == null || box.max() == null) {
            return;
        }
        ...
        for (LaserData_BC8 data : box.laserData) {
            LaserRenderer_BC8.renderLaserStaticRelative(pose, matrix, data, renderOrigin);
        }
    }
    ...
    public static void renderLaserBoxDynamicRelative(Box box, LaserType type, Vec3 renderOrigin, Matrix4f pose,
        Matrix3f normal, VertexConsumer bb, boolean center) {
        if (box == null || box.min() == null || box.max() == null) {
            return;
        }
