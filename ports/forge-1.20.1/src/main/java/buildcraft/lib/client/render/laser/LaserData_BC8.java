...
public class LaserData_BC8 {
    ...
    public static LaserData_BC8 of(LaserType laserType, Vec3 start, Vec3 end, double scale, boolean enableDiffuse, boolean doubleFace, int minBlockLight, int id) {
    	int hash = Objects.hash(laserType, start, end, Double.doubleToLongBits(scale), enableDiffuse, doubleFace, minBlockLight, false);
    	if(hash != hashCache[id]) {
    		objCache[id] = new LaserData_BC8(laserType, start, end, scale, enableDiffuse, doubleFace, minBlockLight, false, hash);
    		hashCache[id] = hash;
    	}
    	return objCache[id];
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (isOrgin != other.isOrgin) return false;
        return true;
    }
