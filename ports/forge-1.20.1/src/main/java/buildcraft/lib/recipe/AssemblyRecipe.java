...
public class AssemblyRecipe extends AssemblyRecipeBasic{
	...
	public Set<ItemStack> getOutputs(IItemHandlerModifiable inputs) {
        return hasRequiredInputs(new RecipeWrapper(inputs))
            ? ImmutableSet.of(output.copy())
            : ImmutableSet.of();
	}

    private boolean hasRequiredInputs(Container container) {
        return IngredientStackAllocation.find(
	...
	public boolean matches(Container container, Level level) {
        return hasRequiredInputs(container);
	}

	@Override
	public ItemStack assemble(Container container, RegistryAccess registryAccess) {
        return hasRequiredInputs(container) ? output.copy() : ItemStack.EMPTY;
	}
