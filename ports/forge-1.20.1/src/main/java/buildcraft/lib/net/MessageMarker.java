...
public class MessageMarker {
    static final int MAX_POSITIONS = Short.MAX_VALUE;
    ...
    public MessageMarker(FriendlyByteBuf buf) {
        ...
        if (count < 0 || count > MAX_POSITIONS || count > buf.readableBytes() / 3) {
            throw new DecoderException("Invalid marker position count: " + count
        ...
        if (buf.isReadable()) {
            throw new DecoderException("Trailing bytes in marker packet: " + buf.readableBytes());
        }
    }
    ...
    public static void toBytes(MessageMarker msg, FriendlyByteBuf buf) {
        ...
        if (msg.count > MAX_POSITIONS) {
            throw new EncoderException("Too many marker positions: " + msg.count);
        }
