...
public class PathConnection extends MarkerConnection<PathConnection> {
    ...
    public boolean canAddMarker(BlockPos from, BlockPos toAdd) {
        ...
        if (positions.getFirst().equals(from)) {
            ...
            if (contains) {
                return positions.getFirst().equals(toAdd);
            } else {
                return true;
            }
        } else {
    ...
    @Override
    @OnlyIn(Dist.CLIENT)
    public void renderInWorld(PoseStack pose, Matrix4f matrix, Vec3 cameraPosition) {
        BlockPos last = null;
        for (BlockPos p : positions) {
            if (last == null) {
                last = p;
            } else {
                renderLaser(pose, matrix, cameraPosition, Vec3.atCenterOf(last), Vec3.atCenterOf(p));
                last = p;
            }
        }
        if (loop) {
            ...
            renderLaser(pose, matrix, cameraPosition, Vec3.atCenterOf(from), Vec3.atCenterOf(to));
        }
    }

    @OnlyIn(Dist.CLIENT)
    private static void renderLaser(PoseStack pose, Matrix4f matrix, Vec3 cameraPosition, Vec3 from, Vec3 to) {
        Vec3 one = offset(from, to);
        Vec3 two = offset(to, from);
        LaserData_BC8 data = new LaserData_BC8(BuildCraftLaserManager.MARKER_PATH_CONNECTED, one, two, RENDER_SCALE, true, false, 0, false);
        LaserRenderer_BC8.renderLaserStaticRelative(pose, matrix, data, cameraPosition);
    }
