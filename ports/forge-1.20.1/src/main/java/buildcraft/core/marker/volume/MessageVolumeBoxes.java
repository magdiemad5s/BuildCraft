...
public class MessageVolumeBoxes {
    ...
    public MessageVolumeBoxes(FriendlyByteBuf buf) {
        ...
        if (buf.isReadable()) {
            throw new DecoderException("Trailing bytes in volume-box snapshot: " + buf.readableBytes());
        }
        buffers = ImmutableList.copyOf(cache);
    }

    public MessageVolumeBoxes(List<VolumeBox> volumeBoxes) {
        if (volumeBoxes.size() > MAX_BOXES) {
            throw new IllegalArgumentException("Too many volume boxes: " + volumeBoxes.size());
        }
