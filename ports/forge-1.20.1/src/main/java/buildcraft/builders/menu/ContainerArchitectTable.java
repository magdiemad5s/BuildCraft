...
//        this.deltaProgress = containerData;
    ...
    public void sendNameToServer(String name) {
        String sanitized = ArchitectNamePolicy.sanitize(name);
    	if (!this.name.equals(sanitized)) {
    		sendMessage(ID_NAME, buffer -> buffer.writeUtf(sanitized, ArchitectNamePolicy.MAX_LENGTH));
    		this.name = sanitized;
    	}
    }
    ...
    public void readMessage(int id, FriendlyByteBuf buffer, LogicalSide side, NetworkEvent.Context ctx) throws IOException {
        ...
        if (side == LogicalSide.SERVER) {
            if (id == ID_NAME && tile != null) {
                tile.setSnapshotName(buffer.readUtf(ArchitectNamePolicy.MAX_LENGTH));
            } else if (id == ID_SETTINGS && tile != null) {
 ...
 /*       if (side == LogicalSide.CLIENT) {
	...
	public boolean stillValid(Player player) {
		if (player.level().isClientSide && getTile() == null) {
			return true;
		}
