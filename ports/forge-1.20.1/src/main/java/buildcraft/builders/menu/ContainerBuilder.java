...
public class ContainerBuilder extends ContainerBCTile<TileBuilder> {
    ...
    public ContainerBuilder(int containerId, Inventory playerInventory, IItemHandlerAdv invSnapshot, IItemHandlerAdv invResources, 
        ...
        TileBuilder builder = getTile();
        List<Tank> tanks = new ArrayList<>(4);
        if (builder == null) {
            // The client block entity can arrive after the menu packet. Keep
            // four correctly ordered widget targets so the authoritative
            // widget payload still has somewhere to decode and render.
            for (int i = 0; i < 4; i++) {
                tanks.add(new Tank("tank" + (i + 1), FluidType.BUCKET_VOLUME * 8, null));
            }
        } else {
            builder.getTankManager().stream().forEach(tanks::add);
        }
        widgetTanks = tanks.stream()
    ...
    private void applyInsertedSnapshotSettings(ItemStack before, net.minecraft.world.entity.player.Player player) {
        ...
        if (!ItemStack.matches(before, after) && after.getItem() instanceof ItemSnapshot
            ...
            TileBuilder builder = getTile();
            if (builder != null) {
                builder.applySnapshotSettingsFromInsertedItem(after, player);
            }
        }
    }
}
