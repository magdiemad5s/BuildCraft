...
public class TileElectronicLibrary extends TileBC_Neptune implements MenuProvider{
    ...
    private static final int MAX_UPLOAD_SESSIONS = 8;
    private final Map<Pair<UUID, Snapshot.Key>, SnapshotUploadAccumulator> upSnapshotSessions = new HashMap<>();
    ...
    public void setSelected(Snapshot.Key newSelected) {
        if (java.util.Objects.equals(selected, newSelected)) {
            return;
        }
        selected = newSelected;
        upSnapshotSessions.clear();
        progressUp = -1;
    ...
    protected void onSlotChange(IItemHandlerModifiable handler, int slot, @Nonnull ItemStack before, @Nonnull ItemStack after) {
        ...
        if (handler == invUpIn || handler == invUpOut) {
            upSnapshotSessions.clear();
        }
    }
    ...
    public void update() {
        ...
        progressDownLast = progressDown;
        pruneExpiredUploadSessions(level.getGameTime());
        progressUpLast = progressUp;
    ...
    private void pruneExpiredUploadSessions(long currentTick) {
        upSnapshotSessions.entrySet().removeIf(entry -> entry.getValue().isExpired(currentTick));
    }
    ...
    public void readPayload(int id, FriendlyByteBuf buffer, LogicalSide side, NetworkEvent.Context ctx) throws IOException {
        ...
        if (side == LogicalSide.SERVER && id == NET_UP) {
            ServerPlayer sender = ctx == null ? null : ctx.getSender();
            Snapshot.Key key = new Snapshot.Key(buffer);
            boolean last = buffer.readBoolean();
            byte[] part = buffer.readByteArray(SnapshotUploadAccumulator.MAX_PART_BYTES);
            if (sender == null || level == null) {
                return;
            }
            ...
            if (selected == null
                ...
                upSnapshotSessions.remove(pair);
                return;
            }

            upSnapshotSessions.entrySet().removeIf(entry ->
                entry.getKey().getLeft().equals(playerId) && !entry.getKey().equals(pair)
            );
            SnapshotUploadAccumulator accumulator = upSnapshotSessions.get(pair);
            if (accumulator == null) {
                if (upSnapshotSessions.size() >= MAX_UPLOAD_SESSIONS) {
                    return;
                }
                accumulator = new SnapshotUploadAccumulator();
                upSnapshotSessions.put(pair, accumulator);
            }
            if (!accumulator.append(part, level.getGameTime())) {
                upSnapshotSessions.remove(pair);
                return;
            }
            if (!last) {
            ...
            byte[] compressed = accumulator.finish();
            upSnapshotSessions.remove(pair);
            try {
                Snapshot uploaded = Snapshot.readFromNBT(NbtSquisher.expand(compressed));
                if (!uploaded.key.equals(key) || key.header == null) {
                    return;
                }
                Snapshot snapshot = uploaded.copy();
                snapshot.key = new Snapshot.Key(snapshot.key, (Snapshot.Header) null);
                snapshot.computeKey();
                if (!Arrays.equals(snapshot.key.hash, key.hash)) {
                    return;
                }
                GlobalSavedDataSnapshots.get(level).addSnapshot(snapshot);
                invUpOut.setStackInSlot(0, ItemSnapshot.getUsed(snapshot.getType(), key.header));
                invUpIn.setStackInSlot(0, StackUtil.EMPTY);
            } catch (IOException | RuntimeException exception) {
                buildcraft.api.core.BCLog.logger.warn("Rejected malformed Electronic Library snapshot upload", exception);
            }
        }
    }
