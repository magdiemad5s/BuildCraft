...
public class TileReplacer extends TileBC_Neptune implements MenuProvider {
    ...
    @Nullable
    private CompoundTag lastSkippedInput;
    ...
    public void update() {
        ...
        CompoundTag inputState = createInputState(snapshotStack, fromStack, toStack);
        if (isSameSkippedInput(lastSkippedInput, inputState)) {
            return;
        }

        Header header = ItemSnapshot.getHeader(snapshotStack);
        if (header == null) {
            rememberSkippedInput(inputState);
            return;
        }
    ...
    static CompoundTag createInputState(ItemStack snapshotStack, ItemStack fromStack, ItemStack toStack) {
    ...
    static boolean isSameSkippedInput(@Nullable CompoundTag previous, CompoundTag current) {
        return previous != null && previous.equals(current);
    }

    private void rememberSkippedInput(CompoundTag inputState) {
        lastSkippedInput = inputState.copy();
    }
