...
//        	level.getChunk(pos).getEventDispatcher(SectionPos.blockToSectionCoord(pos.getY())).unregister(worldEventListener);
    ...
    private void cancelBreakTask(BreakTask breakTask) {
        refundPower(breakTask.paidPower);
    }
    ...
    protected void cancelPlaceTask(PlaceTask placeTask) {
        refundPower(placeTask.paidPower);
    }

    private void refundPower(long paidPower) {
        long availableCapacity = Math.max(0, tile.getBattery().getCapacity() - tile.getBattery().getStored());
        tile.getBattery().addPower(
            Math.min(Math.max(0, paidPower), availableCapacity),
            FluidAction.EXECUTE
        );
    }
    ...
    public Vec3 getPlaceTaskItemPos(PlaceTask placeTask) {
        ...
        double progress = target <= 0
            ? 0
            : Math.max(0, Math.min(1, placeTask.power * 1D / target));
	...
	public boolean tick() {
        ...
        if (level.isClientSide) {
            ...
            if (!breakTasks.isEmpty()) {
                Vec3 newRobotPos = breakTasks.stream()
                    .map(breakTask -> Vec3.atCenterOf(breakTask.pos))
                    .reduce(Vec3.ZERO, Vec3::add)
                    .scale(1D / breakTasks.size());
                newRobotPos = new Vec3(
                    newRobotPos.x,
                    breakTasks.stream().mapToDouble(breakTask -> breakTask.pos.getY()).max().orElse(newRobotPos.y),
                    newRobotPos.z
                ).add(0, 3, 0);
                if (robotPos != null) {
                    newRobotPos = robotPos.add(newRobotPos.subtract(robotPos).scale(0.25D));
                }
        ...
        if (!breakTasks.isEmpty()) {
            for (Iterator<BreakTask> iterator = breakTasks.iterator(); iterator.hasNext(); ) {
                ...
                PowerCharge charge = extractProgressPower(
                    target,
                    breakTask.power,
                    max / breakTasks.size()
                );
                breakTask.power += charge.progress();
                breakTask.paidPower = TaskPowerAccounting.addPaidPower(breakTask.paidPower, charge.payment());
        ...
        if (!placeTasks.isEmpty()) {
            for (Iterator<PlaceTask> iterator = placeTasks.iterator(); iterator.hasNext(); ) {
                ...
                placeTask.power += charge.progress();
                placeTask.paidPower = TaskPowerAccounting.addPaidPower(placeTask.paidPower, charge.payment());
                if (charge.progress() > 0) {
                    renderWork = true;
                }
...
//        if(isDone)
  ...
  //      return isDone;
    ...
    private PowerCharge extractProgressPower(long target, long currentProgress, long maxPower) {
        long remainingProgress = target - currentProgress;
        long wantedPower = TaskPowerAccounting.requestedPayment(
            remainingProgress,
            maxPower,
            POWER_EFFICIENCY_MULTIPLIER
        );
        long extractedPower = tile.getBattery().extractPower(0, wantedPower);
        long progress = TaskPowerAccounting.progressFromPayment(
            remainingProgress,
            extractedPower,
            POWER_EFFICIENCY_MULTIPLIER
        );
        return new PowerCharge(extractedPower, progress);
    }

    private record PowerCharge(long payment, long progress) {
    }
    ...
    public void deserializeNBT(CompoundTag nbt) {
        ...
        try {
            ...
            BCLog.logger.warn("Ignoring malformed persisted Builder tasks", exception);
        }

        // Saved scan results can be stale after a chunk/world reload. Restore the transactions only long enough to
        // return their reserved MJ/items, then rescan the current world rather than replaying an obsolete placement.
        Level level = tile.getWorldBC();
        if (level != null && getBuildingInfo() != null) {
    ...
    public class BreakTask {
        ...
        public BreakTask(CompoundTag nbt) {
            ...
            paidPower = nbt.contains("paidPower") ? nbt.getLong("paidPower") : Math.max(0, power);
        }
        ...
        public CompoundTag writeToNBT() {
            ...
            nbt.putLong("paidPower", paidPower);
            return nbt;
        }
    }
    ...
    public class PlaceTask {
        ...
        public long power;
        private long paidPower;
        private final boolean resourcesExtracted;
        ...
        public PlaceTask(CompoundTag nbt) {
            ...
            power = nbt.getLong("power");
            paidPower = nbt.contains("paidPower") ? nbt.getLong("paidPower") : Math.max(0, power);
            resourcesExtracted = !nbt.contains("resourcesExtracted") || nbt.getBoolean("resourcesExtracted");
        }

        public boolean shouldRefundResources() {
        ...
        public CompoundTag writeToNBT() {
            ...
            nbt.putLong("power", power);
            nbt.putLong("paidPower", paidPower);
            nbt.putBoolean("resourcesExtracted", resourcesExtracted);
            return nbt;
        }
    }
}
