...
public class SchematicEntityDefault implements ISchematicEntity {
    ...
    /** Returns a detached copy translated within a flattened composite snapshot. */
    public SchematicEntityDefault getTranslated(BlockPos offset) {
        SchematicEntityDefault translated = SchematicEntityManager.createCleanCopy(this);
        translated.entityNbt = entityNbt.copy();
        translated.pos = pos.add(Vec3.atLowerCornerOf(offset));
        translated.hangingPos = hangingPos.offset(offset);
        translated.hangingFacing = hangingFacing;
        translated.entityRotation = entityRotation;
        return translated;
    }
