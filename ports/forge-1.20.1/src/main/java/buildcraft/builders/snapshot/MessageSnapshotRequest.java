...
public class MessageSnapshotRequest{
    ...
    public MessageSnapshotRequest(FriendlyByteBuf buf) {
        ...
        if (buf.isReadable()) {
            throw new DecoderException("Trailing bytes in snapshot request: " + buf.readableBytes());
        }
    }
    ...
    public static final BiConsumer<MessageSnapshotRequest, Supplier<NetworkEvent.Context>> HANDLER = (message, ctx) -> {
        ...
        context.enqueueWork(() -> {
            try {
                if (context.getSender() == null || message.key == null
                        || message.key.hash.length != HashUtil.DIGEST_LENGTH) {
                    return;
                }
