...
public enum ClientSnapshots {
    ...
    private static final int REQUEST_RETRY_TICKS = 20 * 5;
    private static final int REQUEST_EXPIRY_TICKS = 20 * 30;
    private static final int MAX_REQUEST_ATTEMPTS = 3;
    private static final int MAX_PENDING_REQUESTS = 128;

    private final List<Snapshot> snapshots = new ArrayList<>();
    private final Map<Snapshot.Key, PendingRequest> pending = new HashMap<>();
    ...
    public Snapshot getSnapshot(Snapshot.Key key) {
        ...
        if (found == null && !pending.containsKey(key)) {
            startRequest(key);
        }
        return found;
    }

    private void startRequest(Snapshot.Key key) {
        if (key == null || Minecraft.getInstance().getConnection() == null) {
            return;
        }
        if (pending.size() >= MAX_PENDING_REQUESTS) {
            Snapshot.Key oldest = pending.entrySet().stream()
                .min((left, right) -> Long.compare(left.getValue().firstTick, right.getValue().firstTick))
                .map(Map.Entry::getKey)
                .orElse(null);
            pending.remove(oldest);
        }
        MessageManager.sendToServer(new MessageSnapshotRequest(key));
        pending.put(key, new PendingRequest(clientTick));
    }
    ...
    public void tick() {
        ...
        List<Snapshot.Key> expired = new ArrayList<>();
        for (Map.Entry<Snapshot.Key, PendingRequest> entry : pending.entrySet()) {
            PendingRequest request = entry.getValue();
            if (clientTick - request.firstTick > REQUEST_EXPIRY_TICKS) {
                expired.add(entry.getKey());
            } else if (request.attempts < MAX_REQUEST_ATTEMPTS
                && clientTick - request.lastSentTick >= REQUEST_RETRY_TICKS) {
                MessageManager.sendToServer(new MessageSnapshotRequest(entry.getKey()));
                request.lastSentTick = clientTick;
                request.attempts++;
            }
        }
    ...
    public void clear() {
        snapshots.clear();
        pending.clear();
        worlds.values().forEach(FakeWorld::clear);
        worlds.clear();
        clientTick = 0;
    }
    ...
    public void onSnapshotReceived(Snapshot snapshot) {
        ...
        FakeWorld oldWorld = worlds.remove(snapshot.key);
        if (oldWorld != null) {
            oldWorld.clear();
        }
    }
    ...
    private static final class PendingRequest {
        ...
        private PendingRequest(long currentTick) {
    ...
    public void renderSnapshot(PoseStack ignoredPose, Snapshot snapshot, int offsetX, int offsetY, int sizeX, int sizeY) {
        ...
        try {
            ...
            modelViewStack.translate(0, 0, -snapshotSize * 2.0F - 3.0F);
            ...
            renderBlocks(minecraft, world, snapshot, modelViewStack, bufferSource);
            if (snapshotSize < 32) {
                renderBlockEntities(minecraft, world, snapshot, partialTick, modelViewStack, bufferSource);
            }
