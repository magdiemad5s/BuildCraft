...
public abstract class Snapshot {
    ...
    public static final int MAX_SNAPSHOT_AXIS = 257;
    public static final int MAX_SNAPSHOT_BLOCKS = 17_000_000;
    ...
    public static int getDataSize(int x, int y, int z) {
        long dataSize = (long) x * y * z;
        if (x <= 0 || y <= 0 || z <= 0
            || x > MAX_SNAPSHOT_AXIS || y > MAX_SNAPSHOT_AXIS || z > MAX_SNAPSHOT_AXIS
            || dataSize > MAX_SNAPSHOT_BLOCKS) {
            throw new IllegalArgumentException(
                "Invalid snapshot dimensions " + x + "x" + y + "x" + z
            );
        }
        return (int) dataSize;
    }
    ...
    public static int validateDimensions(BlockPos size) throws InvalidInputDataException {
        if (size == null) {
            throw new InvalidInputDataException("Snapshot has no dimensions");
        }
        try {
            return getDataSize(size);
        } catch (IllegalArgumentException exception) {
            throw new InvalidInputDataException(exception.getMessage(), exception);
        }
    }
    ...
    public void deserializeNBT(CompoundTag nbt) throws InvalidInputDataException {
        ...
        if (facing == null) {
            throw new InvalidInputDataException("Snapshot has no valid facing");
        }
