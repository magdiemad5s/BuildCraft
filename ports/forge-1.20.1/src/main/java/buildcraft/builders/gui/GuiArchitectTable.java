...
public class GuiArchitectTable extends GuiBC8<ContainerArchitectTable> {
	...
	private boolean initialNameResolved;
	private String unresolvedInitialName = "";
	...
	public void init() {
		...
		nameField.setMaxLength(ArchitectNamePolicy.MAX_LENGTH);
		TileArchitectTable tile = container.getTile();
		unresolvedInitialName = tile == null ? container.name : tile.name;
		initialNameResolved = tile != null;
		nameField.setValue(unresolvedInitialName);
		nameField.setResponder((s) -> {
			if (!suppressNameResponder) container.sendNameToServer(s.trim());
		});
		...
		this.isCreativeButton = this.addRenderableWidget(
				CycleButton.booleanBuilder(Component.translatable("block.architect.allowCreative"), Component.translatable("block.architect.noallowCreative"))
					...
					.displayOnlyValue().withTooltip(b -> Tooltip.create(Component.translatable(b ? "block.architect.tooltip.allowCreative.1" : "block.architect.tooltip.allowCreative.2")))
	...
	protected void drawBackgroundLayer(PoseStack pose, int mouseX, int mouseY, float partialTicks) {
		...
		TileArchitectTable tile = container.getTile();
		drawProgress(guiGraphics, RECT_PROGRESS, ICON_PROGRESS,
				tile == null ? 0 : tile.deltaProgress.getDynamic(partialTicks), 1);
	}

	@Override
	protected void drawForegroundLayer(PoseStack pose, int mouseX, int mouseY) {
		nameField.render(getActiveGraphics(), mouseX, mouseY, 0);
	}

	@Override
	public void containerTick() {
        super.containerTick();
		if (!initialNameResolved && nameField != null) {
			TileArchitectTable tile = container.getTile();
			if (tile != null) {
				initialNameResolved = true;
				if (nameField.getValue().equals(unresolvedInitialName)) {
					suppressNameResponder = true;
					nameField.setValue(tile.name);
