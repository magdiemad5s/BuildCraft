...
public final class PipeApi {
    ...
    public static RedstoneFluxTransferInfo getRfTransferInfo(PipeDefinition def) {
        ...
        return info == null ? RedstoneFluxTransferInfo.DEFAULT : info;
    }
    ...
    /**
     * Per-tick Forge Energy throughput for the legacy RF pipe family.
     * A zero transfer rate explicitly disables the flow.
     */
    public static final class RedstoneFluxTransferInfo {
        public static final RedstoneFluxTransferInfo DEFAULT =
            new RedstoneFluxTransferInfo(100, false);
        ...
        public RedstoneFluxTransferInfo(int transferPerTick, boolean isReceiver) {
            if (transferPerTick < 0) {
                throw new IllegalArgumentException(
                    "transferPerTick must not be negative: " + transferPerTick
                );
            }
            this.transferPerTick = transferPerTick;
            this.isReceiver = isReceiver;
    ...
    static {
        CAP_PIPE = CapabilityManager.get(new CapabilityToken<>(){});
        CAP_PLUG = CapabilityManager.get(new CapabilityToken<>(){});
        CAP_PIPE_HOLDER = CapabilityManager.get(new CapabilityToken<>(){});
        CAP_INJECTABLE = CapabilityManager.get(new CapabilityToken<>(){});
    }
}
