...
public interface ISchematicBlock {
    ...
    /**
     * Builds a block on behalf of an automated, player-owned machine.
     *
     * <p>The default keeps third-party schematic implementations source-compatible. BuildCraft's own schematics
     * override this method so Forge placement/protection events receive the machine owner's fake-player identity.</p>
     *
     * @param owner the machine owner, or {@code null} when no owner is known
     * @param actorPos the position of the machine performing the placement
     */
    default boolean build(Level world, BlockPos blockPos, @Nullable GameProfile owner, BlockPos actorPos) {
